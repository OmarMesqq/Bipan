...
static int (*orig_bsd_signal)(int, sigset_t) = nullptr;
...
static int hook_bsd_signal(int signum, sigset_t handler);
...
static int hook_bsd_signal(int signum, sigset_t handler) {
  if (signum == SIGSYS) {
    write_to_logcat_async(ANDROID_LOG_DEBUG, SIGNALS_TAG, "[bsd_signal] for SIGSYS!", signum);
  }
  return orig_bsd_signal(signum, handler);
}
static int hook_sigprocmask(int how, const sigset_t* set, sigset_t* oldset) {
  if (set) {
    char maskbuf[256];
...
static void dump_sigset(const sigset_t* set, char* out, size_t out_size) {
  ...
  for (int sig = 1; sig < NSIG; sig++) {
    ...
    if (member == 1) {
      ...
      if (written < 0 || (size_t)written >= out_size - used) break;  // truncated
      used += (size_t)written;
    }
  }
