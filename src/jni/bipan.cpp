...
static void log_address_info_extended(const char* label, uintptr_t addr);
...
class Bipan : public zygisk::ModuleBase {
 public:
  ...
  void preAppSpecialize(AppSpecializeArgs* args) override {
    ...
    if (should_spoof) {
      ...
      // Setup shared mem for IPC with the un-seccomped broker
 ...
 private:
  ...
  static void sigsysHandler(int signum, siginfo_t* info, void* ucontext) {
    ...
    uintptr_t pc = uc->uc_mcontext.pc;         // caller's program counter
    ...
    log_address_info_extended("PC (Actual Caller)", pc);
    log_address_info_extended("LR (Return Address)", lr);
    ...
    // Evaluate if the syscall originates from a target library directly in the app
    auto is_target_addr = [](uintptr_t addr, const char* label) -> bool {
      if (addr == 0) {
        LOGD("[%s] Address is 0x0, skipping.", label);
        return false;
      }
      ...
      if (dladdr((void*)addr, &dlinfo) && dlinfo.dli_fname) {
        ...
        bool from_data_dir = (strncmp(path, "/data/app/", 10) == 0 ||
                              strncmp(path, "/data/data/", 11) == 0);
        bool is_apk = (strstr(path, ".apk") != nullptr);

        LOGD("[%s] Resolved path: %s | from_data_dir: %d | is_apk: %d",
             label, path, from_data_dir, is_apk);

        if (from_data_dir && is_apk) {
          LOGD("[%s] TARGET MATCHED!", label);
          return true;
        } else {
          LOGD("[%s] Conditions not met.", label);
          return false;
        }
      } else {
        LOGD("[%s] dladdr failed to resolve address or path is null.", label);
      }
      return false;
    };

    LOGD("--- Evaluating Target ---");
    bool pc_target = is_target_addr(pc, "PC");
    bool lr_target = false;

    // Short-circuiting: only check LR if PC didn't match
    if (!pc_target) {
      lr_target = is_target_addr(lr, "LR");
    } else {
      LOGD("[LR] Skipped checking LR because PC already matched.");
    }

    bool from_target_so = pc_target || lr_target;
    LOGD("Final from_target_so result: %d", from_target_so);
    LOGD("-------------------------");
    ...
    if (from_target_so) {
      ...
      if (syscall_no == __NR_uname) {
        if (ipc_mem->return_value != 0) {
          LOGE("uname failed at broker");
          _exit(1);
        }
        if (arg0 == 0) {
          LOGE("utsname struct for uname is nil");
          _exit(1);
        }
        // Copy the broker's response from shared memory into the app's original pointer
