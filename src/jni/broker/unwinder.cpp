...
bool unwinder(uintptr_t fp, uintptr_t lr, pid_t pid) {
  ...
  if (manual_dladdr(lr, &info, pid)) {
    ...
    if (!is_trusted_lib(info.dli_fname)) {
      write_to_logcat_async(ANDROID_LOG_INFO, TAG, "Very first LR (%p) is a malicious lib: %s. Unwinding over :)", (void*)lr, info.dli_fname);
      close(mem_fd);
      return false;
    }

    write_to_logcat_async(ANDROID_LOG_DEBUG, TAG, "Starting unwinding at LR:  %p -> %-15s | %s (+0x%lx)", (void*)lr, sym_name, info.dli_fname, info.dli_offset);
  } else {
    write_to_logcat_async(ANDROID_LOG_DEBUG, TAG, "Failed to resolve very first LR (%p)!", (void*)lr);
    close(mem_fd);
    return false;
  }
  ...
  for (int i = 0; i < MAX_STACK_TRACE; ++i) {
    if (!fp || (fp & 0x7)) {
       ...
       */
      write_to_logcat_async(ANDROID_LOG_ERROR, TAG, "Unwinder exhausted: frame ptr is null/misaligned!");
      close(mem_fd);
      return true;
    }
    ...
    if (
       ...
       */
      write_to_logcat_async(ANDROID_LOG_ERROR, TAG, "Unwinder exhausted: couldn't read remote frame at %p", (void*)fp);
      close(mem_fd);
      return true;
    }
    ...
    if (!return_addr) {
       ...
       */
      write_to_logcat_async(ANDROID_LOG_ERROR, TAG, "Unwinder exhausted: null return addr");
      close(mem_fd);
      return true;
    }
    ...
    if (manual_dladdr(return_addr, &info, pid)) {
      find_label_in_elf(info.dli_fname, info.dli_offset, sym_name, sizeof(sym_name));
      write_to_logcat_async(ANDROID_LOG_DEBUG, TAG, "  Ancestor:      %p -> %-15s | %s (+0x%lx)\n", (void*)return_addr, sym_name, info.dli_fname, info.dli_offset);

      if (!is_trusted_lib(info.dli_fname)) {
        write_to_logcat_async(ANDROID_LOG_INFO, TAG, "Found malicious lib: %s at %p after %d unwindings", info.dli_fname, (void*)return_addr, i);
        close(mem_fd);
        return false;
      }
    } else {
    ...
    if (next_fp <= fp) {
       ...
       */
      write_to_logcat_async(ANDROID_LOG_ERROR, TAG, "Unwinder exhausted: FP not increasing");
      close(mem_fd);
      return true;
    }
  ...
  close(mem_fd);
  write_to_logcat_async(ANDROID_LOG_WARN, TAG, "Walked %d frames and found only safe libs. Allowing!", MAX_STACK_TRACE);
  return true;
}
...
static void find_label_in_elf(const char* path, uintptr_t offset, char* out_name, size_t max_len) {
  if (!path) {
    write_to_logcat_async(ANDROID_LOG_ERROR, TAG, "find_label_in_elf: got empty path. early returning!");
    return;
  }
  ...
  struct stat st;
  if (fstat(fd, &st) < 0) {
    write_to_logcat_async(ANDROID_LOG_ERROR, TAG, "find_label_in_elf: Failed to fstat fd: %d (from %s)", fd, path);
    close(fd);
    return;
  }
  ...
  close(fd);
  if (map == MAP_FAILED) {
    write_to_logcat_async(ANDROID_LOG_ERROR, TAG, "find_label_in_elf: Failed to mmap fd-backed mem");
    return;
  }
  ...
  if (memcmp(ehdr->e_ident, ELFMAG, SELFMAG) != 0) {
    write_to_logcat_async(ANDROID_LOG_WARN, TAG, "find_label_in_elf: %s not an ELF. Not searching symbols.", path);
...
static bool manual_dladdr(uintptr_t addr, ManualDlInfo* info, pid_t pid) {
  ...
  if (!f) {
    write_to_logcat_async(ANDROID_LOG_WARN, TAG, "Failed to open remote's %s", proc_pid_maps_path);
    return false;
  }
